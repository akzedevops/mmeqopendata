...
├── backfill.py                     # one-shot event backfill helper
...
```bash
pytest tests/ -v          # Python suite (80 tests)
...
seismology, clustering, and dam-risk scoring. The Go suite additionally pins
byte-level parity with the Python writer/geocoder via golden files generated by
