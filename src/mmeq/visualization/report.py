...
def generate_pdf_report(
    ...
    if risk_df is not None and not risk_df.empty:
        mainshock_mag = forecast_params.get("mainshock_mag", "?") if forecast_params else "?"
        pdf.section_title("4. Dam Seismic Risk Assessment")
        pdf.body_text(
            f"Risk assessment for {len(risk_df)} dams using PGA estimation from the M{mainshock_mag} mainshock, "
