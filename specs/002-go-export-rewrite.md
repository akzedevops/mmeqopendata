...
### Increment plan (updated 2026-07-02 — API v2 is live)

The mmeq API v2 (Go/SQLite, `github.com/akzedevops/mmeq-api`, private) is in production
and the nightly CI already fetches through it, so the Go exporter targets **v2 first**:
typed JSON, half-open `[from,to)` windows, `limit`/`offset` pagination, `updated_after`
incremental sync. `catalog.Validate` accepts both the v1 string shape and v2 native
numbers, so a v1 fallback stays possible. The 001 bisection tripwire still applies
(`MMEQ_API_PAGE_CAP`, default off — v2 paginates properly so it should never fire).

- [x] **I1** — `internal/config` (MMEQ_* env mirror) + `internal/catalog` model,
      `Validate`, `Dedup` (keep-last by id); unit-tested.
- [ ] **I2** — `internal/api`: v2 client — `FetchWindow(from, to)` (v1-inclusive dates,
      converted to v2 half-open, paginated) and `FetchUpdatedAfter(since)`; retries with
      backoff; httptest-tested.
- [ ] **I3** — `internal/catalog` writer: monthly (overwrite) / yearly / combined
...
- [ ] **I4** — geocoder port or hybrid Python post-step (per table above).
- [ ] **I5** — `cmd/mmeq-export` wiring + shadow CI job (Go alongside Python ≥3 daily
      cycles, clean diffs) → cutover of `daily_data_fetch.yml`.
